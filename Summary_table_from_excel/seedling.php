...
function seedlings(){
    ...
    $user_qry = "SELECT DISTINCT(forest_user_name) FROM seedling";
    ...
    foreach ($user_list_array as $users) {
        $cf_qry = "SELECT DISTINCT(forest_regime_cfcode) FROM seedling WHERE forest_user_name = '".$users."'";
        // echo $cf_qry."\n";
    ...
    foreach ($forest_list_array as $username => $forests) {
        ...
        foreach ($forests as $forest) {
            ...
            $block_qry = "SELECT DISTINCT(forest_block_block_num) FROM seedling WHERE forest_regime_cfcode = '".$forest."' AND forest_user_name = '".$username."'";
            ...
            while($row = $block_list->fetch_assoc()) {
                ...
                $tree_query = "SELECT DISTINCT(species_vernacular_name) FROM seedling WHERE forest_block_block_num='".$block_name."' AND forest_regime_cfcode ='".$forest."' AND forest_user_name = '".$username."'";
                ...
                while($tree_row = $tree_list->fetch_assoc()) {
                    ...
                    $summary_report_query = "SELECT SUM(counth) as sum_counth FROM seedling WHERE species_vernacular_name = '".$tree_name."' AND forest_block_block_num='".$block_name."' AND forest_regime_cfcode ='".$forest."' AND forest_user_name = '".$username."'";
    ...
    db_to_excel_seedling($usr_forest_block);
    $drop_seedling_table = "DROP TABLE IF EXISTS seedling;";
    $conn->query($drop_seedling_table);
}
?>