...
$user_qry = "SELECT DISTINCT(forest_user_name) FROM sample";
...
foreach ($user_list_array as $users) {
    $cf_qry = "SELECT DISTINCT(forest_regime_cfcode) FROM sample WHERE forest_user_name = '".$users."'";
    // echo $cf_qry."\n";
...
foreach ($forest_list_array as $username => $forests) {
    ...
    foreach ($forests as $forest) {
        ...
        $block_qry = "SELECT DISTINCT(forest_block_block_num) FROM sample WHERE forest_regime_cfcode = '".$forest."' AND forest_user_name = '".$username."'";
        ...
        while($row = $block_list->fetch_assoc()) {
            ...
            $tree_query = "SELECT DISTINCT(species_vernacular_name) FROM sample WHERE forest_block_block_num='".$block_name."' AND forest_regime_cfcode ='".$forest."' AND forest_user_name = '".$username."'";
            ...
            while($tree_row = $tree_list->fetch_assoc()) {
                ...
                $summary_report_query = "SELECT SUM(counth) as sum_counth, SUM(volumeh) as sum_volumeh, SUM(timberh) as sum_timberh, SUM(firewoodh) as sum_firewood, SUM(co2h) as sum_co2h FROM sample WHERE species_vernacular_name = '".$tree_name."' AND forest_block_block_num='".$block_name."' AND forest_regime_cfcode ='".$forest."' AND forest_user_name = '".$username."'";
