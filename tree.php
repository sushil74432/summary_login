...
function trees(){
    global $conn;
    $user_qry = "SELECT DISTINCT(forest_user_name) FROM tree";
    ...
    foreach ($user_list_array as $users) {
        $cf_qry = "SELECT DISTINCT(forest_regime_cfcode) FROM tree WHERE forest_user_name = '".$users."'";
        // echo $cf_qry."\n";
    ...
    foreach ($forest_list_array as $username => $forests) {
        ...
        foreach ($forests as $forest) {
            ...
            $block_qry = "SELECT DISTINCT(forest_block_block_num) FROM tree WHERE forest_regime_cfcode = '".$forest."' AND forest_user_name = '".$username."'";
            ...
            while($row = $block_list->fetch_assoc()) {
                ...
                $tree_query = "SELECT DISTINCT(species_vernacular_name) FROM tree WHERE forest_block_block_num='".$block_name."' AND forest_regime_cfcode ='".$forest."' AND forest_user_name = '".$username."'";
                ...
                while($tree_row = $tree_list->fetch_assoc()) {
                    ...
                    $summary_report_query = "SELECT SUM(counth) as sum_counth, SUM(volumeh) as sum_volumeh, SUM(timberh) as sum_timberh, SUM(firewoodh) as sum_firewood, SUM(co2h) as sum_co2h FROM tree WHERE species_vernacular_name = '".$tree_name."' AND forest_block_block_num='".$block_name."' AND forest_regime_cfcode ='".$forest."' AND forest_user_name = '".$username."'";
    ...
    db_to_excel_tree($usr_forest_block);
    $drop_tree_table = "DROP TABLE IF EXISTS tree;";
    $conn->query($drop_tree_table);
    compress_all(); // this function must be called from the file present as last member of the array $file_list in function export_filename() in upload.php    
}
?>